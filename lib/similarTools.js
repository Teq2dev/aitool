...
export async function getSimilarTools(targetSlug, limitStrong = 5, limitRelated = 3) {
  ...
  if (!toolA) {
    ...
    try {
      ...
      const targetTool = await toolsCollection.findOne({ slug: targetSlug });
      
      if (!targetTool) {
        return { strongSimilar: [], relatedTools: [] };
      }
      
      const targetCategories = targetTool.categories || [];
      const targetTags = targetTool.tags || [];
      const targetSlugRef = targetTool.slug;
      
      let fallbackTools = [];
      
      // Try category match first (case-insensitive via regex)
      if (targetCategories.length > 0) {
        const categoryRegexes = targetCategories.slice(0, 2).map(c => 
          new RegExp(`^${c.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i')
        );
        fallbackTools = await toolsCollection
          .find({ 
            categories: { $in: categoryRegexes },
            slug: { $ne: targetSlugRef },
            status: 'approved' 
          })
          .sort({ votes: -1, rating: -1 })
          .limit(limitStrong + limitRelated + 2)
          .project({ slug: 1, name: 1, shortDescription: 1, logo: 1, categories: 1, pricing: 1, votes: 1, rating: 1 })
          .toArray();
      }
      
      // If category search returns fewer than 3 tools, broaden with tag match
      if (fallbackTools.length < 3 && targetTags.length > 0) {
        const tagTools = await toolsCollection
          .find({ 
            tags: { $in: targetTags.slice(0, 3) },
            slug: { $nin: [targetSlugRef, ...fallbackTools.map(t => t.slug)] },
            status: 'approved' 
          })
          .sort({ votes: -1, rating: -1 })
          .limit(limitStrong + limitRelated - fallbackTools.length)
          .project({ slug: 1, name: 1, shortDescription: 1, logo: 1, categories: 1, pricing: 1, votes: 1, rating: 1 })
          .toArray();
        fallbackTools = [...fallbackTools, ...tagTools];
      }
      
      // Last resort: top rated tools from any category
      if (fallbackTools.length < 3) {
        const topTools = await toolsCollection
          .find({ 
            slug: { $nin: [targetSlugRef, ...fallbackTools.map(t => t.slug)] },
            status: 'approved',
            featured: true
          ...
          .project({ slug: 1, name: 1, shortDescription: 1, logo: 1, categories: 1, pricing: 1, votes: 1, rating: 1 })
          .toArray();
        fallbackTools = [...fallbackTools, ...topTools];
      }
        
      const formattedFallback = fallbackTools.slice(0, limitStrong + limitRelated).map(t => ({
        slug: t.slug,
        name: t.name,
