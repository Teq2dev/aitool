  ...
  try {
    ...
    if (search) {
      ...
      try {
        ...
        // Fallback to robust regex search
    ...
    return {
      tools: JSON.parse(JSON.stringify(fallbackTools)),
...
export async function getCategories() {
  try {
    ...
    return JSON.parse(JSON.stringify(sampleCategories.slice(0, 20).map(c => ({
      ...c,
...
export async function getFeaturedTools(limit = 6) {
  try {
    ...
    return JSON.parse(JSON.stringify(sampleTools.filter(t => t.featured).slice(0, limit)));
  }
}
...
export async function getTrendingTools(limit = 10) {
  try {
    ...
    return JSON.parse(JSON.stringify(sampleTools.filter(t => t.trending).slice(0, limit)));
  }
}