...
export default async function ImageConverterPage({ searchParams }) {
  ...
  const lang = params?.lang || 'en';
  const localizedData = tool.translations?.[lang] || tool;
  const schemas = getImageToolSchema('image-converter', lang, localizedData);
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 pb-20">
        ...
        <section className="max-w-5xl mx-auto px-4 pt-10 pb-8 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 text-purple-700 dark:text-purple-400 text-xs font-semibold mb-4">
            <RefreshCw className="w-3.5 h-3.5" />
            <span>Universal In-Browser Image Transcoding</span>
          </div>
