...
const toolData = getPdfToolData('pdf-to-excel');
...
export async function generateMetadata({ searchParams }) {
  ...
  const slug = toolData.slug || 'pdf-to-excel';
  return {
...
export default async function PdfToExcelPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = toolData.translations?.[lang] || toolData;
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
        ...
        <PdfRelatedTools currentSlug="pdf-to-excel"  lang={lang} />
      </div>
    </>
  );
}
