...
export default async function WebpToPngPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = tool.translations?.[lang] || tool;
  const schemas = getImageToolSchema('webp-to-png', lang, localizedData);
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 pb-20">
        ...
        <section className="max-w-5xl mx-auto px-4 pt-10 pb-8 text-center">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400 text-xs font-semibold mb-4">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Lossless PNG Rasterization & Alpha Channel Preservation</span>
          </div>
