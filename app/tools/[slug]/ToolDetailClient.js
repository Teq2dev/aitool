...
export default function ToolDetailClient({ initialTool, initialStrongSimilar = [], initialRelatedTools = [], initialLang = 'en', relatedBlogs = [], relatedCats = [] }) {
      ...
      <div className="mt-12 space-y-8 border-t border-gray-100 pt-8">
        ...
        {relatedCats && relatedCats.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {relatedCats.map(rc => (
                <Link key={rc} href={`/${effectiveLang === 'en' ? '' : effectiveLang + '/'}categories/${rc}`} className="px-4 py-2 bg-gray-50 hover:bg-blue-50 text-gray-700 hover:text-blue-700 rounded-full text-sm font-medium transition-colors border border-gray-100">
                  {rc.replace(/-/g, ' ')}
                </Link>
              ))}
            </div>
        ...
        {(strongSimilar.length > 0 || relatedTools.length > 0) && (
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {strongSimilar.concat(relatedTools || []).slice(0, 10).map(st => (
                <Link key={st.slug} href={`/${effectiveLang === 'en' ? '' : effectiveLang + '/'}tools/${st.slug}`} className="flex items-center p-3 rounded-xl border border-gray-100 hover:border-purple-200 hover:shadow-md transition bg-white group">
