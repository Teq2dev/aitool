...
export default async function ImageResizerPage({ searchParams }) {
  ...
  const lang = params?.lang || 'en';
  const localizedData = tool.translations?.[lang] || tool;
  const schemas = getImageToolSchema('image-resizer', lang, localizedData);
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 pb-20">
        ...
        <section className="max-w-5xl mx-auto px-4 pt-10 pb-8 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-400 text-xs font-semibold mb-4">
            <Maximize2 className="w-3.5 h-3.5" />
            <span>High-Precision Bicubic Resampling</span>
          </div>
        ...
        {/* Comprehensive SEO Content, Guide, Architecture & FAQs */}
