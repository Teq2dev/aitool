...
const toolData = getPdfToolData('pdf-to-png');
...
export async function generateMetadata({ searchParams }) {
  ...
  const slug = toolData.slug || 'pdf-to-png';
  return {
...
const ToolClientComponent = dynamic(() => import('../pdf-to-jpg/PdfToImageClient'), {
  
  loading: () => <PdfToolSkeleton />,
});

export default async function PdfToPngPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = toolData.translations?.[lang] || toolData;
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
        ...
        <ToolClientComponent toolSlug="pdf-to-png" format="png" defaultFormatLabel="PNG" />

        {/* Educational SEO Content Section */}
        <PdfToolContentSection tool={localizedData} lang={lang} />

        {/* Related PDF Utilities */}
        <PdfRelatedTools currentSlug="pdf-to-png"  lang={lang} />
      </div>
    </>
  );
}
