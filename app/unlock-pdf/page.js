...
const toolData = getPdfToolData('unlock-pdf');
...
export async function generateMetadata({ searchParams }) {
  ...
  const slug = toolData.slug || 'unlock-pdf';
  return {
...
const ToolClientComponent = dynamic(() => import('./UnlockPdfClient'), {
  
  loading: () => <PdfToolSkeleton />,
});

export default async function UnlockPdfPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = toolData.translations?.[lang] || toolData;
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
        ...
        <PdfRelatedTools currentSlug="unlock-pdf"  lang={lang} />
      </div>
    </>
  );
}
