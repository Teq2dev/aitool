...
const toolData = getPdfToolData('compress-pdf');
...
export async function generateMetadata({ searchParams }) {
  ...
  const slug = toolData.slug || 'compress-pdf';
  return {
...
const ToolClientComponent = dynamic(() => import('./CompressPdfClient'), {
  
  loading: () => <PdfToolSkeleton />,
});

export default async function CompressPdfPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = toolData.translations?.[lang] || toolData;
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
        ...
        <PdfRelatedTools currentSlug="compress-pdf"  lang={lang} />
      </div>
    </>
  );
}
