...
const toolData = getPdfToolData('pdf-to-jpg');
...
export async function generateMetadata({ searchParams }) {
  ...
  const slug = toolData.slug || 'pdf-to-jpg';
  return {
...
const ToolClientComponent = dynamic(() => import('./PdfToImageClient'), {
  
  loading: () => <PdfToolSkeleton />,
});

export default async function PdfToJpgPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = toolData.translations?.[lang] || toolData;
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
        ...
        <PdfRelatedTools currentSlug="pdf-to-jpg"  lang={lang} />
      </div>
    </>
  );
}
