...
const toolData = getPdfToolData('powerpoint-to-pdf');
...
export async function generateMetadata({ searchParams }) {
  ...
  const slug = toolData.slug || 'powerpoint-to-pdf';
  return {
...
const ToolClientComponent = dynamic(() => import('./ClientComponent'), {
  
  loading: () => <PdfToolSkeleton />,
});

export default async function PowerpointToPdfPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = toolData.translations?.[lang] || toolData;
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
        ...
        <PdfRelatedTools currentSlug="powerpoint-to-pdf"  lang={lang} />
      </div>
    </>
  );
}
