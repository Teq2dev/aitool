...
const ImageRotatorClient = dynamic(() => import('./ImageRotatorClient'), {
...
export default async function ImageRotatorPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = tool.translations?.[lang] || tool;
  const schemas = getImageToolSchema('image-rotator', lang, localizedData);
      ...
      <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 pb-20">
        ...
        <section className="max-w-5xl mx-auto px-4 pt-10 pb-8 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-50 dark:bg-violet-950/40 border border-violet-200 dark:border-violet-800 text-violet-700 dark:text-violet-400 text-xs font-semibold mb-4">
            ...
            <span>90Â° / 180Â° / 270Â° Reorientation & Flipping</span>
          </div>
