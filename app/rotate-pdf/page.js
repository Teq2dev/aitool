...
const toolData = getPdfToolData('rotate-pdf');
...
export async function generateMetadata({ searchParams }) {
  ...
  const slug = toolData.slug || 'rotate-pdf';
  return {
...
const ToolClientComponent = dynamic(() => import('./RotatePdfClient'), {
  
  loading: () => <PdfToolSkeleton />,
});

export default async function RotatePdfPage({ searchParams }) {
  const params = await searchParams;
  const lang = params?.lang || 'en';
  const localizedData = toolData.translations?.[lang] || toolData;
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
        ...
        <PdfRelatedTools currentSlug="rotate-pdf"  lang={lang} />
      </div>
    </>
  );
}
